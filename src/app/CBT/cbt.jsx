...
function Portal() {
    ...
    <div className="w-full min-h-screen flex flex-col lg:grid lg:grid-cols-12 bg-[#f7f7f7]">
      ...
      <div className="flex flex-col w-full lg:col-span-9 min-h-screen">
        ...
        <div className="flex-1 w-full bg-white flex flex-col lg:grid lg:grid-cols-10">
          ...
          {/* <div className="questionArea bg-[#e7e7e750] col-span-6 w-full flex items-center justify-center p-6 min-h-[50vw] lg:min-h-0 lg:h-full">
            <div className="relative w-full h-[55vw] sm:h-[45vw] lg:h-full rounded-lg overflow-hidden">
              
              <LatexRenderer text={fullTest?.[0]?.[0]?.paper?.MathsPaper[13]?.question} />
  ...
  <div className="relative w-full h-[55vw] sm:h-[45vw] lg:h-full rounded-lg overflow-hidden">
    ...
    {/* QUESTION */}
    <LatexRenderer text={fullTest?.[0]?.[0]?.paper?.MathsPaper[20]?.question} />
    ...
    >
      {Array.from({ length: 20 }).map((_, i) => (
        ...
        >
          ...
          <div className="optionArea col-span-4 p-4 flex flex-col items-start gap-2 border-t lg:border-t-0 lg:border-l border-gray-200">
            <h2 className="text-black font-bold mb-2 text-sm lg:text-base">Choose correct answer</h2>
            
            {fullTest?.[0]?.[0]?.paper?.MathsPaper[22]?.options 
              ? Object.entries(fullTest[0][0].paper.MathsPaper[22].options).map(([key, value]) => (
                  <button
                    key={key}
                    className="w-full text-left p-2 border text-black text-sm cursor-pointer rounded mb-1 hover:bg-teal-100 transition"
                  >
                    ● {key}. <LatexRenderer text={value} />
                  </button>
                ))
              : [1,2,3,4].map((_, index) => (
                  <button
