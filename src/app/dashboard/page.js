...
export default function Dashboard() {
  ...
  const navItems = [
    ...
    { label: 'My complains', icon: <ErrorIcon fontSize="small"/> },
  ...
  return (
    <div className="flex flex-col lg:grid lg:grid-cols-12 h-screen overflow-hidden">
      ...
      <div className="hidden lg:flex flex-col gap-4 col-span-2 p-4 h-full bg-teal-700">

        {/* Logo */}
        <div className="flex items-center gap-2">
          <Image src="/logo1.png" className="rounded" width={30} height={30} alt="logo" />
          <h1 className="text-sm font-semibold text-white whitespace-nowrap">Skylite Educations</h1>
        </div>
      ...
      {/* ── MAIN CONTENT ── */}
      <div className="flex flex-col flex-1 lg:col-span-10 h-screen overflow-hidden">
        ...
        {/* Page Content */}
        <div className="flex-1 overflow-auto pb-16 lg:pb-0 bg-[#e7e7e7]">
          {navigationOptions === 'All Tests'    && <Tests />}
          {navigationOptions === 'My Tests'     && <MyTests />}
          {navigationOptions === 'Ask Doubts'   && <AskDoubtForm />}
          {navigationOptions === 'Events'       && <ExamDashboard />}
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-teal-700 flex justify-around items-center h-16 px-2 border-t border-teal-600">
        {navItems.slice(0, 5).map(({ label, icon }) => (
          <button
            key={label}
            onClick={() => setNavigationOption(label)}
            className={`flex flex-col items-center gap-0.5 px-2 py-1 rounded-lg transition
              ${navigationOptions === label ? 'text-yellow-300' : 'text-white/70'}`}
          >
            {icon}
        ...
        <div className="relative group">
          <button className="flex flex-col items-center gap-0.5 px-2 py-1 text-white/70">
            <SettingsIcon fontSize="small"/>
            <span className="text-[10px] font-medium">More</span>
          </button>
          <div className="absolute bottom-16 right-0 bg-teal-800 rounded-xl shadow-xl hidden group-focus-within:flex flex-col min-w-[140px] overflow-hidden">
